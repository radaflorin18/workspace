...
int main()
{
    ...
    do { ret = scanf_s("%u", &select); getchar(); }
        while (!(1 == ret)); //compare with the number of written parameters

    printf("Enter the two operands: a and b \n");

    //Some input validation
    do { ret = scanf_s("%d %d", &a, &b); getchar(); }
        while (!(2 == ret)); //compare with the number of written parameters
